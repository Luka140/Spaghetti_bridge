...
def assemble_K(E, areas, lengths, moi, connections, node_positions):
...
def simulate_frame(E, node_pos, elements, connections, BC, forces):
    K = assemble_K(E, areas=elements[:,2], lengths=elements[:,1], moi=elements[:,3], connections=connections, node_positions=node_pos)
    ...
    u = np.zeros((node_pos.shape[0]) * 3)
    u[BC[0, :] - 1] = BC[1, :]
    u[[idx for idx in range((node_pos.shape[0]) * 3) if idx not in BC[0, :]]] = u_global
...
if __name__=="__main__":
    ...
    sym_constraint_idx_top = [(node_pos.shape[0] - 1) * 3, (node_pos.shape[0] - 1) * 3 + 2]
    ...
    BC_idx =  [0, 3, 4, 5] + sym_constraint_idx_top
    BC_disp = [0, 0, 0, 0, 0, 0]
    BC = np.array([BC_idx, BC_disp])

    mass = 1000000  # kg
    force_vector = np.zeros(node_pos.shape[0]*3)
    ...
    if plotting:
        disp = displacements.reshape(-1, 3)
        new_pos_viz = node_pos + disp[:, :2] * disp_vis_scale
